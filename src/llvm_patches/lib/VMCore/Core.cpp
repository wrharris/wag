...
/* wrharris extensions */
LLVMValueRef LLVMCallGetCalledFunction(LLVMValueRef Call)
{
  return wrap(static_cast<CallInst*>(unwrap(Call))->getCalledFunction());
}

LLVMBasicBlockRef LLVMGetNormalDest(LLVMValueRef Invoke) {
  return wrap(static_cast<InvokeInst*>(unwrap(Invoke))->getNormalDest());
}

LLVMBasicBlockRef LLVMGetUnwindDest(LLVMValueRef Invoke) {
  return wrap(static_cast<InvokeInst*>(unwrap(Invoke))->getUnwindDest());
}

unsigned LLVMGetNumSuccessors(LLVMValueRef Term) {
  return (static_cast<TerminatorInst*>(unwrap(Term)))->getNumSuccessors();
}

LLVMBasicBlockRef LLVMGetSuccessor(LLVMValueRef Term, unsigned indx)
{
  return wrap(static_cast<TerminatorInst*>(unwrap(Term))->getSuccessor(indx));
}

int LLVMGetAddr(LLVMValueRef Valu)
{
  long un = (long) Valu;
  int i = (int) un;
  return i;
...
LLVMValueRef LLVMInvokeGetCalledFunction(LLVMValueRef Invoke)
{
  return wrap(static_cast<InvokeInst*>(unwrap(Invoke))->getCalledFunction());
}
