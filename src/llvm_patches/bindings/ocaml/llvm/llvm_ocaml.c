...
/* wrharris: extensions */
/* llvalue -> llvalue */
CAMLprim LLVMValueRef llvm_call_get_called_function(LLVMValueRef Call)
{
  CAMLparam0();
  LLVMValueRef Called_func = LLVMCallGetCalledFunction(Call);
  if (Called_func) {
    value Option = alloc(1, 0);
    Field(Option, 0) = (value) Called_func;
    CAMLreturn(Option);
  }
...
/* llvalue -> llvalue */
CAMLprim LLVMBasicBlockRef llvm_get_normal_dest(LLVMValueRef Invoke)
{
  return LLVMGetNormalDest(Invoke);
}

CAMLprim value llvm_get_addr(LLVMValueRef Valu)
{
  CAMLparam0();
  CAMLreturn(Val_int(LLVMGetAddr(Valu)));
}

CAMLprim LLVMBasicBlockRef llvm_get_unwind_dest(LLVMValueRef Invoke)
{
  return LLVMGetUnwindDest(Invoke);
}

/* llvalue -> int */
CAMLprim value llvm_get_num_successors(LLVMValueRef Term) {
  return Val_int(LLVMGetNumSuccessors(Term));
}

CAMLprim LLVMBasicBlockRef llvm_get_successor(LLVMValueRef Term, value Indx) {
  return LLVMGetSuccessor(Term, Int_val(Indx));
}

CAMLprim LLVMValueRef llvm_invoke_get_called_function(LLVMValueRef Invoke) {
  CAMLparam0();
  LLVMValueRef Called_func = LLVMInvokeGetCalledFunction(Invoke);
  if (Called_func) {
    value Option = alloc(1, 0);
    Field(Option, 0) = (value) Called_func;
    CAMLreturn(Option);
